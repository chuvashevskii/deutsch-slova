...
const VIEWS = [
  ...
  { key: "head", label: "Одно слово — разные значения" },
  { key: "translation", label: "Один перевод — разные слова" },
] as const;
...
const Value = ({
  ...
  return (
    <span
      className={kind === "old" ? "text-muted line-through" : "font-semibold"}
    >
      {text}
    </span>
  );
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
        ...
        {row.rank ? (
          <span className="rounded-full border border-line px-2 py-0.5 font-mono text-[9.5px] text-faint">
            №{row.rank}
          </span>
        ) : (
          <span className="rounded-full border border-dashed border-line px-2 py-0.5 font-mono text-[9.5px] text-faint">
            вне списка
          </span>
        )}
        <span
        ...
        {row.is_draft ? (
          <span className="whitespace-nowrap rounded-full bg-surface-2 px-2 py-0.5 font-mono text-[9.5px] uppercase tracking-wider text-faint">
            черновик
          </span>
        ) : null}
      </div>
      ...
      <p className="mt-2 text-[13.5px] leading-relaxed">
        ...
        <Value text={row.old_value} kind="old" />{" "}
...
export const ReviewPage = () => {
  ...
  const byTable = useMemo(() => {
    const map = new Map<EditTable, EditRow[]>(
      EDIT_TABLES.map((name) => [name, []]),
    );
  ...
  if (isError)
    return <LoadError what="журнал правок" onRetry={() => void refetch()} />;

  const rows = (byTable.get(table) ?? []).filter((row) =>
    matchesMode(row, mode),
  );
  const open = (byTable.get(table) ?? []).filter(
    (row) => row.reverted_at === null,
  ).length;
    ...
    <div className="py-4">
      <header className="rounded-xl border border-line bg-surface p-4">
        ...
        <p className="mt-1 text-[12.5px] leading-relaxed text-muted">
          Разбор колоды на однозначность ответа. Правки уже в словаре — строка
          показывает, что было и что стало, и позволяет вернуть прежнее.
        </p>
        {data?.truncated ? (
          <p className="mt-2 rounded-lg bg-bad/10 px-3 py-2 text-[12.5px] text-bad">
            Журнал длиннее, чем помещается на экран, — показана только последняя
            часть.
          </p>
        ) : null}
      </header>
