...
describe('навигация', () => {
  it('«Как пользоваться» стоит первым среди редких: новому человеку он нужнее', () => {
    const { sidebar } = renderLayout();
    const rare = labels(sidebar).slice(3);
    expect(rare[0]).toBe('Как пользоваться');
  });

  it('в боковой колонке все разделы и нет «Ещё»', () => {
    const { sidebar } = renderLayout();
