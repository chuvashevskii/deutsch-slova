-- Спряжение у карточки, заведённой руками.
--
-- Функция `create_word` колонок `form_ich`…`form_ihr` не знала вовсе:
-- в её `insert` их не было, и глагол рождался без единой формы. В колоде
-- таких нет ни одного — из 710 глаголов у 699 заполнены все пять, —
-- так что заведённый руками был бы первым.
--
-- Поля необязательные, и это не послабление. Пустое лицо в колоде
-- значит «такой формы не бывает»: `regnen`, `schneien`, `es gibt`
-- живут только в третьем лице, и одиннадцать глаголов заполнены
-- частично именно поэтому. Требовать все пять значило бы требовать
-- выдумать «ich regne».
--
-- Тело функции — прежнее, слово в слово; добавлены только пять колонок.
-- Проверки, список частей речи и поиск двойника не трогались.
...
begin
  ...
  insert into public.words (
    ...
    form_ich, form_du, form_er, form_wir, form_ihr,
...
comment on function public.create_word is
  'Заводит карточку руками. Только администратору; карточка рождается черновиком и в «Учить» не попадает. Спряжение необязательно: пустое лицо значит «формы не бывает».';
