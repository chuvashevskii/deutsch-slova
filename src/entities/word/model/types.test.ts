...
describe('стилистическая помета', () => {
  ...
  it('называет нейтральный наравне с остальными', () => {
    // Помета стоит только там, где у слова есть сосед по переводу.
    // Промолчать об одном из группы — вернуть ту самую неоднозначность.
    expect(registerLabel('neutral · нейтральный')).toBe('нейтральный');
  });

  it('показывает и помету, которой нет в разборе', () => {
    expect(registerLabel('derb · грубый')).toBe('грубый');
    expect(registerLabel('gehoben · возвышенный')).toBe('возвышенный');
  });

  it('разбирает помету без русской половины — базу до миграции', () => {
    expect(registerLabel('umgangssprachlich')).toBe('разговорный');
    expect(registerLabel('neutral')).toBe('нейтральный');
    expect(registerLabel('derb')).toBe('грубый');
  });
